...
class WeightLoader(object):
    ...
    def __init__(self, cfgfile, weights_file_path):
...
#         self.total_conv_idx = self.get_all_conv_layer_idx(model.module_defs)
        
    def get_all_conv_layer_idx(self, module_defs):
        CBL_idx = []
    ...
    def _create_param_tensors(self, conv_params, param_category, suffix):
        ...
        # print("param_data_shape  ", param_data_shape)
    ...
    def _load_one_param_type(self, conv_params, param_category, suffix):
        ...
        print("param_category   ", param_category)
        ...
        print("np.array(param_shape)  ", np.array(param_shape))
        
        param_size = np.product(np.array(param_shape))
        # print("param_size   ", param_size)
        module_idx = int(param_name.split("_")[0])-1
        my_module = self.model.module_list[module_idx]

        if param_category == 'bn':
            if suffix == 'mean':
                param_data = my_module[1].running_mean.cpu().detach().numpy()
            elif suffix == 'bias':
                param_data = my_module[1].bias.cpu().detach().numpy()
            elif suffix == 'scale':
                param_data = my_module[1].weight.cpu().detach().numpy()
            elif suffix == 'var':
                param_data = my_module[1].running_var.cpu().detach().numpy()
        elif param_category == 'conv':
            if suffix == 'weights':
                param_data = my_module[0].weight.cpu().detach().numpy()
            elif suffix == 'bias':
                param_data = my_module[0].bias.cpu().detach().numpy()
...
class GraphBuilderONNX(object):
    ...
    def build_onnx_graph(
        ...
        weight_loader = WeightLoader(cfg_file_path, weights_file_path)
        initializer = list()
...
def main():
    ...
    parser.add_argument("--model_def", type=str, default="./config/yolov3-polyp-416.cfg", help="path to model definition file")
    # parser.add_argument("--data_config", type=str, default="config/coco.data", help="path to data config file")
    parser.add_argument("--weights_path", type=str, default="weights/yolov3.weights", help="path to weights file")
    parser.add_argument("--img_size", type=float, default=416, help="size of each image dimension")
    parser.add_argument("--yolo_last_conv_filters_num", type=int, default=18, help="number of last convolution layer filters")
    ...
    output_tensor_dims['082_convolutional'] = [opt.yolo_last_conv_filters_num, int(opt.img_size/32), int(opt.img_size/32)]
    output_tensor_dims['094_convolutional'] = [opt.yolo_last_conv_filters_num, int(opt.img_size/16), int(opt.img_size/16)]
    output_tensor_dims['106_convolutional'] = [opt.yolo_last_conv_filters_num, int(opt.img_size/8), int(opt.img_size/8)]
...
#     output_tensor_dims['106_convolutional'] = [18, 76, 76]
    ...
    yolov3_model_def = builder.build_onnx_graph(
        ...
        weights_file_path=opt.weights_path,
        verbose=True)
    ...
    # output_file_name = opt.weights_path.split("/")[-1].split(".pt")[0]
    output_file_name = opt.weights_path.split(".pt")[0]
    # Serialize the generated ONNX graph to this file:
    output_file_path = output_file_name + '.onnx'
